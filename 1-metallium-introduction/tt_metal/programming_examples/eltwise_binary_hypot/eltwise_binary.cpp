...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::vector<bfloat16> b_data(elements_per_tile * n_tiles, bfloat16(1.f));
        ...
        auto reader = CreateKernel(
            program,
            OVERRIDE_KERNEL_PREFIX "eltwise_binary_hypot/kernels/dataflow/read_tiles.cpp",
            core,
        ...
        auto writer = CreateKernel(
            program,
            OVERRIDE_KERNEL_PREFIX "eltwise_binary_hypot/kernels/dataflow/write_tile.cpp",
            core,
        ...
        auto compute = CreateKernel(
            program,
            OVERRIDE_KERNEL_PREFIX "eltwise_binary_hypot/kernels/compute/tiles_hypot.cpp",
            core,
        ...
        constexpr float eps = 0.05f; // loose tolerance because of the nature of bfloat16
        TT_FATAL(result_vec.size() == a_data.size(), "Result vector size mismatch");
        for (size_t i = 0; i < result_vec.size(); ++i) {
            const float expected = std::hypotf(static_cast<float>(a_data[i]), static_cast<float>(b_data[i]));
